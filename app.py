...
def get_second_level_folders(root_folder):
    """Retrieves all first-layer and second-layer
    subfolders inside the root folder."""
    if not os.path.exists(root_folder):
        return [], {}

    first_level_folders = [d for d in os.listdir(root_folder) if os.path.isdir(
...
def main():
    ...
    if not first_level_folders:
        st.sidebar.warning(
            "No first-layer folders found in the root directory.")
        return

    first_layer_selected = st.sidebar.selectbox(
        "Select a First-Layer Folder", first_level_folders)
    ...
    if first_layer_selected and first_layer_selected in second_level_folders:
        ...
        second_layer_selected = st.sidebar.selectbox(
            "Select a Second-Layer Folder",
            second_layer_folders) if second_layer_folders else None
    else:
        second_layer_selected = None

    st.title("PDF & Excel Manager")
    st.write(f"**Root Path:** {root_folder}")
    st.write(f"**First-Layer Folder:**"
             f" {first_layer_selected if first_layer_selected else 'None'}")
    st.write(f"**Second-Layer Folder:** "
             f"{second_layer_selected if second_layer_selected else 'None'}")

    if second_layer_selected:
        selected_folder = os.path.join(root_folder, first_layer_selected,
                                       second_layer_selected)
